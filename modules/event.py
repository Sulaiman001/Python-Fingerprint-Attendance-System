...
class DBManager:
    ...
    def get_event(self, event_id):
        if event_id == "":
            print _("You must specify the id of the event")
            return
        self.__cursor.execute("SELECT * FROM events WHERE id=%s", event_id)
        row = self.__cursor.fetchone()
        if row == None:
            print _("Not met the required event")
            return None       
        else:
...
class EventSection:
    def __init__(self, parent_window, parent_box, config_param, cursor):
        ...
        frame_event_list = gtk.Frame(_("Registered Events"))
        ...
        #Add column to list
        self.__events_treeview.append_column(gtk.TreeViewColumn(_("No."), gtk.CellRendererText(), text=0))
        self.__events_treeview.append_column(gtk.TreeViewColumn(_("Name"), gtk.CellRendererText(), text=1))
        self.__events_treeview.append_column(gtk.TreeViewColumn(_("Date"), gtk.CellRendererText(), text=2))
        self.__events_treeview.append_column(gtk.TreeViewColumn(_("Academic"), gtk.CellRendererText(), text=3))      
        ...
        frame_event_fields = gtk.Frame(_("Facts Matter"))
        ...
        #Academic event?
        self.__chb_academic_events = gtk.CheckButton(_("Academic Event"))
        ...
        b_clean_entries = gtk.Button(_("Clear Fields"))
        ...
        if not itera == None:
            if self.__manager != None:
                ...
                if event_data != None:                            
                    ...
                    self.__b_save_data.set_label("Update Data")
            else:
                self.__show_error_dialog(_("Can not edit the event data, it is not connected to the database."))
        else:
            self.__show_error_dialog(_("You must select an event for editing"))
    ...
    def __delete_event_cb(self, button):
        ...
        if not itera == None:
            ...
            if delete_result_ok:
                ...
                self.__show_error_dialog(_("An error occurred while trying to delete event Database"))
        else:
            self.__show_error_dialog(_("Please select an event to remove"))
    ...
    def __clean_event_entry_fields(self, widget=None):
        ...
        self.__e_credits.set_sensitive(False)
        self.__chb_academic_events.set_active(False)
        self.__b_save_data.set_label(_("Record data"))
    ...
    def __save_event_cb(self, button):
        if self.__completed_mandatory_fields():
            if self.__manager != None:
                ...
                if button.get_label() == _("Record Data"):                    
                    save_result_ok = self.__manager.save_event(event_data)
                    if save_result_ok:
                        self.__add_events_to_list()
                        self.__clean_event_entry_fields()
                    else:
                        self.__show_error_dialog(_("An error occurred while trying to register the event in the Database"))                    
                elif button.get_label() == _("Update Data"):
                    update_result_ok = self.__manager.update_event(self.__event_id, event_data)
                    if update_result_ok:
                        self.__add_events_to_list()
                    else:
                        self.__show_error_dialog(_("An error occurred while trying to update the event in the Database"))
                    self.__clean_event_entry_fields()
                    button.set_label(_("Record Data"))
            else:
                self.__show_error_dialog(_("Not record the event data, it is not connected to the database")) 
        else:
            if self.__e_credits.get_text() == "" or self.__cb_type.get_active_text() == None:
                self.__show_error_dialog(_("Please enter the number of credits and the type of event"))
            else:
    ...
    def __delete_all_events_cb(self, button):
        q_dialog = gtk.MessageDialog(self.__parent_window, gtk.DIALOG_MODAL, \
                                     ...
                                     _("All system events will be deleted. Do you want to continue with the action?"))
