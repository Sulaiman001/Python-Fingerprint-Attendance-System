...
class ScanWindow(threading.Thread):
    ...
    def __init__(self, main_window, dev, config_param, chb_fp):
        ...
        self.__window.set_title(_("Register Fingerprint"))
        ...
        label_title = gtk.Label(_("To register a fingerprint, you must correctly swipe ")\
                                  + finger_name + " " + hand_name + " " + str(self.__nr_enroll_stages) + _(" times.")) 
        ...
        self.__label_message = gtk.Label(_("Slide your finger now"))
        vbox.pack_start(self.__label_message,False,False,5)
        hbox = gtk.HBox(False,5)        
        self.__cancel_button = gtk.Button(" " + _("Cancel") + " ")
    ...
    def __set_up_widgets(self):
        ...
        self.__label_message.set_text(_("Slide your finger now"))
    ...
    def __enroll(self):
        ...
        gobject.idle_add(self.__update_progressbar, progress_fraction, \
                         _("Step " + str(stage) + "/" + str(self.__nr_enroll_stages)))
        while stage != self.__nr_enroll_stages:            
            ...
            if result[1] != None:
                ...
                gobject.idle_add(self.__update_label, _("Slide your finger now"))
                if stage == self.__nr_enroll_stages:
                    gobject.idle_add(self.__update_label, _("Completed!"))
            else:
                gobject.idle_add(self.__update_label, \
                                 _("Error in step " + str(stage+1) + " the registration of its mark, try again"))
...
class DBManager:
    ...
    def get_user(self, user_id):
        if user_id == "":
            print _("To find a user must specify the userid")
            return
        self.__cursor.execute("SELECT * FROM people WHERE id=%s", user_id)
        row = self.__cursor.fetchone()
        if row == None:
            print _("No user found.")
            return None       
        else:
...
class UserSection:
    def __init__(self, parent_window, parent_box, config_param, cursor, dev):
        ...
        frame_user_list = gtk.Frame(_("Registered Users"))
        ...
        self.__r_name = gtk.RadioButton(None, _("by First Name"))
        self.__r_name.set_active(True)
        search_options_hbox.pack_start(self.__r_name, False, False, 0)
        self.__r_last_name = gtk.RadioButton(self.__r_name, _("by Last Name"))
        search_options_hbox.pack_start(self.__r_last_name, False, False, 0)
        self.__r_dni = gtk.RadioButton(self.__r_name, _("by Number"))
        ...
        #Add column to list
        self.__user_treeview.append_column(gtk.TreeViewColumn(_("No."), gtk.CellRendererText(), text=0))
        self.__user_treeview.append_column(gtk.TreeViewColumn(_("Name"), gtk.CellRendererText(), text=1))
        self.__user_treeview.append_column(gtk.TreeViewColumn(_("Document"), gtk.CellRendererText(), text=2))
        self.__user_treeview.append_column(gtk.TreeViewColumn(_("User"), gtk.CellRendererText(), text=3))
        ...
        frame_user_fields = gtk.Frame(_("User Data"))
        ...
        self.__b_image = gtk.Button(_("Add"))
        ...
        label_finger_print = gtk.Label(_("Fingerprint"))
        ...
        b_get_finger_print = gtk.Button(_("Record"))
    ...
    def __show_error_dialog(self, message=""):
        ...
        e_dialog.set_title(_("Fingerprint Attendance System"))
        e_dialog.run()
        e_dialog.destroy()
    ...
    def __edit_user_cb(self, button):
        ...
        if not itera == None:
            if self.__manager != None:
                ...
                self.__show_error_dialog(_("Can not edit user data, it is not connected to the database"))
        else:
            self.__show_error_dialog(_("You must select a user to edit"))
    ...
    def __delete_user_cb(self, button):
        ...
        if not itera == None:
            ...
            if delete_result_ok:
                ...
                self.__show_error_dialog(_("An error occurred while trying to remove the user from the database"))
        else:
            self.__show_error_dialog(_("You must select a user to remove"))
                ...
                if not self.__manager.exists_user_name(self.__e_user_name.get_text()):
                    if not self.__manager.exists_dni(self.__e_dni.get_text()):
                        ...
                        if save_result_ok:
                            ...
                            self.__show_error_dialog(_("An error occurred attempting to write user data"))                        
                    else:
                        self.__show_error_dialog(_("Not record the user data record number already exists in the database"))
                else:
                    self.__show_error_dialog(_("Can not record user data, the user name already exists in the database"))
            else:
                self.__show_error_dialog(_("Can not record user data, it is not connected to the database")) 
        else:
            self.__show_error_dialog(_("Please complete the fields marked with * before recording data"))
    ...
    def __delete_all_user_cb(self, button):
        q_dialog = gtk.MessageDialog(self.__parent_window, gtk.DIALOG_MODAL, \
                                     ...
                                     _("All users of the system will be deleted. Do you want to continue with the action?"))
        q_dialog.set_title(_("Fingerprint Attendance System"))
    ...
    def __open_scan_window_cb(self, button):                        
        if self.__dev != None:
            ...
            self.__show_error_dialog(_("The device was not initiated so can not be registered fingerprints"))
