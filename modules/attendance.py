...
class ScanWindow(threading.Thread):
    ...
    def __init__(self, main_window, config_param, dev, parent):
        ...
        label_title = gtk.Label(_("Slide your finger ") + finger + " " + hand + _(" by fingerprint reader")) 
...
class AttendanceRegistryResult(threading.Thread):
    ...
    def __init__(self, main_window):
        ...
        self.__window = gtk.Window(gtk.WINDOW_TOPLEVEL)
        self.__window.set_title(_("Attendance Record Result"))
        self.__window.set_position(gtk.WIN_POS_CENTER)
        self.__window.set_modal(True)
        self.__window.connect("delete_event", self.delete_event)
        self.__window.set_destroy_with_parent(True)       
...
class DBManager():
    ...
    def record_attendance(self, user_name="", dni="", event_id=-1):
        ...
        if row == None:
            print _("No user found required")
            return False
        user_id = row[0]
    ...
    def get_user_info(self, user_id="", user_dni=""):
        if user_id != "":
            ...
            print _("To search the fingerprint of a user must specify the username or number.")
            return None
        row = self.__cursor.fetchone()
        if row == None:
            print _("The user does not have fingerprint")
            return None       
        else:
...
class AttendanceSection:
    def __init__(self, parent_window, parent_box, cursor, config_manager, dev, event_id):
        ...
        title_label = gtk.Label(_("Check In / Out"))
    ...
    def __attendance_by_dni(self):        
        label = gtk.Label(_("Enter your card number and press Return (Enter)"))
        ...
        label = gtk.Label(_("Enter your Number"))
    ...
    def __attendance_by_user_name(self):        
        label = gtk.Label(_("Enter your username and press Return (Enter)"))
        self.__attendance_box.add(label)
    ...
    def __pre_record_attendance(self, widget):
        if self.__attendance_dni:
            ...
            if self.__user_dni == "":
                self.__show_dialog(message=_("You must enter a card number"))
                return
            ok_dni = self.__db_manager.exists_dni(dni=self.__user_dni)
            if not ok_dni:
                self.__show_dialog(message=_("The card number entered does not exist in the system"))
                return
            ...
            if self.__user_name == "":
                self.__show_dialog(message=_("You must enter a username"))
                return    
            ok_user_name = self.__db_manager.exists_user_name(user_name=self.__user_name)
            if not ok_user_name:
                self.__show_dialog(message=_("The user entered does not exist, enter a user name registered in the system"))
                return
        ...
        if self.__attendance_finger_print:            
            if user_data["finger_print"] != "":                
                ...
                self.__show_dialog(message=_("The user has not entered your fingerprint registered in the database"))
        else:
    ...
    def record_attendance(self):
        ...
        if result != False:                        
            if result == "entrance":
                message = self.__name + " " + self.__last_name + _(" Entry is registered properly!")
            else:
                message = self.__name + " " + self.__last_name + _(" Its output is correctly registered!")
            ...
            message = _("An error occurred while trying to register his / her, try again")
