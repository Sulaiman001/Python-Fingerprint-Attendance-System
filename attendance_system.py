...
class AttendanceSystem:
    def delete_event(self, widget, event, data=None):             
        if self.is_admin or self.admin_ok(title=_("Close App")):
            return False            
        else:
            return True
    ...
    def __init__(self, finger_print_device, database, db_cursor,\
        ...
        self.window.set_title(_("Attendance System"))
        ...
        if response == gtk.RESPONSE_ACCEPT:
            if r_password == self.admin_password:
                ...
                e_dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, gtk.MESSAGE_ERROR, \
                                             gtk.BUTTONS_OK, message_format=_("Incorrect Password!"))
            ...
            if self.__user_section != None:            
                if not self.__user_section.unsave_datas():
                    ...
                    q_dialog = gtk.MessageDialog(self.window, gtk.DIALOG_MODAL, \
                                                ...
                                                "There are unsaved data if you leave the application will be lost, continue?")
    ...
    def admin_cb(self, button):
        if self.is_admin or self.admin_ok():
            ...
            self.b_add_user = gtk.Button(_("Users"))
            self.b_add_user.connect('clicked', self._users_cb)
            self.b_events = gtk.Button(_("Events"))
            self.b_events.connect('clicked', self._events_cb)
            self.b_report = gtk.Button(_("Information"))
            self.b_report.connect('clicked', self._reports_cb)
            self.b_config = gtk.Button(_("Settings"))
    ...
    def __show_select_event_window(self):
        dialog = gtk.Dialog("Fingerprint Attendance System", self.window,\
                            gtk.DIALOG_MODAL, (gtk.STOCK_OK, gtk.RESPONSE_OK, \
                            gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL))
        dialog.set_default_size(640,480)
        dialog.vbox.pack_start (gtk.Label(_("Select an event and press Ok")), False, False, 5)
        #Search Options
        search_vbox = gtk.VBox()
        frame_search = gtk.Frame(_("Search"))
        ...
        #Add column to list
        events_treeview.append_column(gtk.TreeViewColumn(_("Name"), gtk.CellRendererText(), text=1))
    ...
    def create_main_toolbar(self):
        ...
        self.tb_attendance.set_tooltip_text(_("Control Attendance (Ctrl+A)"))
        ...
        #Admin Options
        self.tb_config = gtk.ToolButton(label=_("Administrator"))
        self.tb_config.set_expand(True)
        self.tb_config.connect('clicked', self.admin_cb)
        self.tb_config.set_tooltip_text(_("Configure Application (Ctrl+D)"))
        ...
        self.tb_exit.set_tooltip_text(_("Close App (Ctrl+E)"))
...
class SetupApplication():
    ...
    def __init__(self, splash_screen):
        ...
        splash_screen.update_progressbar(_("Loading Settings..."))
    ...
    def start(self, splash_screen):
        if self.__config_param != None:
            splash_screen.update_progressbar(_("Starting our database ..."))
            ok_db = self.__init_database(self.__config_param["database"])
            if ok_db:
                splash_screen.update_progressbar(_("Looking Fingerprint Device ..."))
                self.__init_finger_print_device()
                if self.__config_param["app"]["attendance_by_finger_print"] == "True":                    
                    if self.__dev != None:
                        splash_screen.update_progressbar(_("Starting Fingerprint device ..."))
                        self.__dev.open()
            else:
                return self.__setup_errors        
        else:
            self.__setup_errors["config"] = True
            self.__setup_errors["mcfg"] = _("There was a problem when trying to access the settings, the application will close.")    
        return self.__setup_errors
    ...
    def __open_finger_print_device(self):
        try:
            ...
            self.__setup_errors["mfp"] = _("There was a problem when trying to start the fingerprint device, the application will not work properly.")
    def __connect_db(self, db_param):
        try:
            ...
            self.__setup_errors["mdb"] = _("There was a problem while trying to connect to the database, the application will close")
            return False
